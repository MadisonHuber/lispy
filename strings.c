...
// function for constructing string lval
    ...
    while (a->count > 0) {
        ...
        if (strcmp(op, "%") == 0) {
            if (y->num == 0) {
                ...
                x = lval_err("Modulus by Zero--not a number!");
                break;
            }
             x->num %= y->num; 
        }
...
int lval_eq(lval* x, lval* y) {
    ...
    switch (x->type) {
        ...
        /* If builtin compare, otherwise compare foramls and body */
        case LVAL_FUN:
...
// if function
// get user to pass in result of a comparison, Q-expression for code to be evaluated on true, Q-expression for code to be evaluated on false
...
// function that can load and evaluate a file when passed a string of its name
...
// builtin print function that will output data
...
// builtin error function that takes in a user supplied string and provides it as an error message for lval_err
...
lval* lval_read(mpc_ast_t* t) {
    ...
    /* If Symbol of Number or String return conversion to that type */
    ...
    for (int i = 0; i < t->children_num; i++) {
        ...
        // above line must be strstr NOT strcmp
...
int main(int argc, char** argv)
{
   ...
   if (argc == 1) { 
        ...
        while (1)
        {
            ...
            add_history(input); 
            // add input to history, from Editline
            ...
            free(input);
             // Free retrieved input, with Editline
